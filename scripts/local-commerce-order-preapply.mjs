...
const expectedSchema=appliedMode?18:13;
...
const draft=readFileSync('local/commerce/migrations/0014_local-commerce-order-commit.sql','utf8');
...
if(appliedMode) rows.products[0].fulfillment_definition.requiresProductionPreview=physical;
...
if(mixed) {
 ...
 if(appliedMode) product.fulfillment_definition.requiresProductionPreview=false;
...
const numericTax=structuredClone(facts);numericTax.amounts.tax.amount=0;
const missingAddress=structuredClone(facts);missingAddress.contact={email:'preapply@example.invalid'};
const invalidEmail=structuredClone(facts);invalidEmail.contact.email='invalid';
...
${check(`(${rpc('commit',numericTax)})->>'status'<>'found'`,'numeric tax accepted')}
${check(`(${rpc('commit',invalidEmail)})->>'status'<>'found'`,'invalid contact accepted')}
${physical?check(`(${rpc('commit',missingAddress)})->>'status'<>'found'`,'physical address exemption'):''}
result:=${rpc('commit')};
...
${check(`(SELECT tax_status='not_activated' AND tax_amount_cents IS NULL AND total_cents=subtotal_cents+shipping_cents-discount_cents FROM local_commerce.order_purchase_snapshots WHERE project_id=${q(project)} AND owner_id=${q(owner)})`,'header tax/conservation')}
${check(`NOT EXISTS(SELECT 1 FROM local_commerce.order_item_purchase_snapshots WHERE project_id=${q(project)} AND owner_id=${q(owner)} AND (pricing_snapshot->'tax') IS DISTINCT FROM '{"status":"not_activated","amount":null}'::jsonb)`,'item tax/null')}
RAISE NOTICE 'rollback-only Order preapply checks PASS';
END $checks$;
ROLLBACK;
DO $after$ BEGIN
${check(`(SELECT count(*) FROM local_commerce.migration_ledger)=${expectedSchema}`,'ledger changed')}
${appliedMode?check(`to_regclass('local_commerce.order_creation_bindings') IS NOT NULL AND NOT EXISTS(SELECT 1 FROM pg_trigger WHERE tgname='preapply_exact_owner_failure')`,'applied state/fixture trigger incorrect'):[
check(`to_regclass('local_commerce.order_creation_bindings') IS NULL`,'draft table survived'),
check(`NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='local_commerce' AND p.proname IN ('order_commit','order_canonical_allocations'))`,'draft functions survived'),
check(`EXISTS(SELECT 1 FROM pg_constraint WHERE conrelid='local_commerce.access_grants'::regclass AND conname='access_grants_capability_key')`,'original uniqueness not restored')].join('\n')}
END $after$;`;
...
console.log(JSON.stringify({mode:appliedMode?'APPLIED FUNCTION / ROLLED-BACK FIXTURES':'PRE-APPLY ROLLBACK ONLY',classification:mixed?'mixed':physical?'physical':'digital-only / email-only contact',exit:run.status,error:run.error?.code,stdout:run.stdout,stderr:run.stderr}));

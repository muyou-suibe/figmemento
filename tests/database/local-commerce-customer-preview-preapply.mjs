...
const candidate=readFileSync('local/commerce/migrations/0022_local-commerce-customer-preview-decisions.sql','utf8');
const entry={version:22,migrationId:'local-commerce-customer-preview-decisions',filename:'0022_local-commerce-customer-preview-decisions.sql',
 ...
 forwardFix:'After permanent application use 0023+ only on authorized run-5576dfd8. Never edit applied 0001-0022. No Task 8 or provider activation.'};
const planned={...manifest,schemaVersion:22,migrations:[...manifest.migrations,entry]};
...
try {
 sql(wrapper.replace('begin;','begin;set local statement_timeout=\'8000ms\';').replace(/commit;\s*$/,
  `${customerPreviewSqlChecks(project,prep.markerDigest)}${customerPreviewFaultSql(project,prep.markerDigest)} rollback;`));
} finally { assert.equal(snapshot(),before,'complete table digests unchanged after rollback'); }
assert.equal(sql('select count(*) from local_commerce.migration_ledger;'),'21');
console.info(JSON.stringify({status:'PASS',scope:'rollback-only pre-apply; NOT real HTTP or Task 7.3 acceptance',run,ledger:21,pending:0,

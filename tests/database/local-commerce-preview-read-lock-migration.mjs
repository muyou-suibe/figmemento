...
assert.equal(ledger.length,22);assert.ok([22,23].includes(manifest.schemaVersion));
const registered=manifest.migrations.find(m=>m.version===23);
manifest.schemaVersion=22;manifest.migrations=manifest.migrations.slice(0,22);
...
const candidate=readFileSync('local/commerce/migrations/0023_local-commerce-preview-read-lock.sql','utf8');
const entry={version:23,migrationId:'local-commerce-preview-read-lock',filename:'0023_local-commerce-preview-read-lock.sql',
 checksum:createHash('sha256').update(candidate).digest('hex'),
 rollback:'Disable persistent customer preview actions and preserve all Orders, private media, manifests, decisions and ledger. No reset, reseed, deletion or purchase rewrite.',
 forwardFix:'After permanent application use 0024+ only on authorized run-5576dfd8. Never edit applied 0001-0023. No Task 8 or provider activation.'};
if(registered)assert.deepEqual(registered,entry);
const planned={...manifest,schemaVersion:23,migrations:[...manifest.migrations,entry]};
...
try {
 sql(wrapper.replace('begin;','begin;set local statement_timeout=\'8000ms\';').replace(/commit;\s*$/,
  `${customerPreviewSqlChecks(project,prep.markerDigest)}${customerPreviewFaultSql(project,prep.markerDigest)}${fulfillmentReplaySql()} rollback;`));
} finally { assert.equal(snapshot(),before,'complete table digests unchanged after rollback'); }
assert.equal(sql('select count(*) from local_commerce.migration_ledger;'),'22');
console.info(JSON.stringify({status:'PASS',scope:'rollback-only pre-apply; NOT real HTTP or Task 7.4 acceptance',run,ledger:22,pending:0,
 ...
 guestMemberSqlAuthorization:true,noteUtf16:true,upstreamAndHistoricalImmutability:true,aclRls:true,ledgerWrapper:true}));

if(process.argv.includes('--apply-authorized-0023')){
 ...
 assert.equal(after.length,23);planned.migrations.forEach((m,i)=>{assert.equal(after[i].version,m.version);assert.equal(after[i].checksum,m.checksum);});
 console.info(JSON.stringify({status:'PASS',applied:23,ledger:23,pending:0,checksum:entry.checksum}));
}

...
assert.equal(ledger.length,23);assert.ok([23,24].includes(manifest.schemaVersion));
const registered=manifest.migrations.find(m=>m.version===24);
manifest.schemaVersion=23;manifest.migrations=manifest.migrations.slice(0,23);
...
const candidate=readFileSync('local/commerce/migrations/0024_local-commerce-production-quality.sql','utf8');
const entry={version:24,migrationId:'local-commerce-production-quality',filename:'0024_local-commerce-production-quality.sql',
 ...
 forwardFix:'After permanent application use 0025+ only on authorized run-5576dfd8. Never edit applied 0001-0024. No Task 8 or provider activation.'};
if(registered)assert.deepEqual(registered,entry);
const planned={...manifest,schemaVersion:24,migrations:[...manifest.migrations,entry]};
...
try {
 sql(wrapper.replace('begin;','begin;set local statement_timeout=\'8000ms\';').replace(/commit;\s*$/,
  `${customerPreviewSqlChecks(project,prep.markerDigest)}${customerPreviewFaultSql(project,prep.markerDigest)}${fulfillmentReplaySql()}${productionQualitySql(project,prep.markerDigest)} rollback;`));
} finally { assert.equal(snapshot(),before,'complete table digests unchanged after rollback'); }
assert.equal(sql('select count(*) from local_commerce.migration_ledger;'),'23');
console.info(JSON.stringify({status:'PASS',scope:'rollback-only pre-apply; NOT real HTTP or Task 7.4 acceptance',run,ledger:23,pending:0,
...
if(process.argv.includes('--apply-authorized-0024')){
 ...
 assert.equal(after.length,24);planned.migrations.forEach((m,i)=>{assert.equal(after[i].version,m.version);assert.equal(after[i].checksum,m.checksum);});
 console.info(JSON.stringify({status:'PASS',applied:24,ledger:24,pending:0,checksum:entry.checksum}));
}

...
const args={p_project_id:project,p_marker_digest:prep.markerDigest,p_actor_kind:'operator',p_actor_id:'local-development-operator',p_public_reference:'FM-LOCAL-0000000000000000',p_operation:'read',p_fulfillment_id:null,p_expected_version:null,p_key_digest:null,p_input:{}};
const results=[];
for(const [role,credential] of [['anon',info.ANON_KEY],['authenticated',jwt],['service_role',info.SERVICE_ROLE_KEY]]){
 ...
 const r=await fetch(info.API_URL+'/rest/v1/rpc/fulfillment_preview_command',{method:'POST',headers,body:JSON.stringify(args),signal:AbortSignal.timeout(5000)});
 ...
 for(const table of ['fulfillment_preview_media','preview_manifest_entries'])for(const method of ['GET','POST','PATCH','DELETE']){
...
console.info(JSON.stringify({status:'PASS',run,ledger:21,pending:0,signature:sig,securityDefiner:true,fixedSearchPath:true,rlsTables:2,privateBucket:true,publicObjectHttp:publicObject.status,results}));

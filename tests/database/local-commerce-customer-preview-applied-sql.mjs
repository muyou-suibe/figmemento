// Applied 0022 validation. Every synthetic SQL fixture/fault is rolled back.
...
assert.equal(ledger.length,22);assert.equal(manifest.schemaVersion,22);
...
try {sql(`begin;set local statement_timeout='8000ms';${customerPreviewSqlChecks(project,prep.markerDigest)}${customerPreviewFaultSql(project,prep.markerDigest)} rollback;`);}
finally {assert.equal(snapshot(),before,'complete tables unchanged after rollback');}
console.info(JSON.stringify({status:'PASS',run,ledger:22,pending:0,unchangedTables:tables.length,customerFaultPoints:4,revisionPublicationFaultPoints:8,guestMemberSqlAuthorization:true,utf16Boundaries:true,aclRls:true,historicalUpstreamImmutable:true}));

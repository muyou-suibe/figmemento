...
try {
 ...
 for(const mode of ['disabled','required','mixed']){
  const o=mode==='disabled'?await admitted([disabledHandoff]):await pending(mode==='mixed'?[handoff,disabledHandoff]:[handoff]);
  const startInput=()=>({fulfillmentActionId:randomUUID(),actionKind:'start_production',expectedAggregateVersion:version(o)});
  const early=startInput();
  await checked(await lifecycle(o,{...early,expectedAggregateVersion:undefined}),400);
  await checked(await customer(o,{...early}),400);
  let approvalRace=null;
  if(mode!=='disabled'){
   await checked(await lifecycle(o,early),409);
   if(mode==='required'){
    const approvalInput=action(o,'approve_preview',1),competingStart=startInput();
    const pair=await Promise.all([customer(o,approvalInput),lifecycle(o,competingStart,1)]);
    const pairStatuses=pair.map(response=>response.status);assert.deepEqual([...pairStatuses].sort(),[200,409]);
    for(const response of pair)await response.arrayBuffer();approvalRace={kind:'start-production-vs-approval',pids:[a.pid,b.pid],statuses:pairStatuses};
   }else await checked(await customer(o,action(o,'approve_preview',1)));
  }
  const productionInput=startInput(),other={...productionInput,fulfillmentActionId:randomUUID()},racePids=[a.pid,b.pid];
  const responses=await Promise.all([lifecycle(o,productionInput,0),lifecycle(o,other,1)]),statuses=responses.map(r=>r.status);
  const bodies=await Promise.all(responses.map(r=>r.json()));
  assert.deepEqual([...statuses].sort(),[200,409]);
  const winner=statuses[0]===200?productionInput:other,original=bodies[statuses.indexOf(200)];
  assert.equal(original.fulfillment.status,'in_production');
  const quality={fulfillmentActionId:randomUUID(),actionKind:'mark_quality_check',expectedAggregateVersion:version(o)};
  const qr=await checked(await lifecycle(o,quality));assert.equal(qr.fulfillment.status,'quality_check');
  await restart(mode+' production/quality');
  await exact(o,mode+' production replay after quality',()=>lifecycle(o,winner),original,'fulfillment');
  await exact(o,mode+' quality replay',()=>lifecycle(o,quality),qr,'fulfillment');
  await checked(await lifecycle(o,{...quality,fulfillmentActionId:randomUUID(),expectedAggregateVersion:version(o)}),409);
  const before=allEffects(o);await stop(b);b=start(1,'disabled');await ready(b,1);
  const denied=await lifecycle(o,winner,1);assert.ok([403,404].includes(denied.status));await denied.arrayBuffer();
  assert.equal(allEffects(o),before);await stop(b);b=start(1);await ready(b,1);
  const counts=audits(o);for(const row of counts)assert.equal(row.count,row.audits);
  assert.equal(counts.find(row=>row.kind==='start_production').count,1);
  assert.equal(counts.find(row=>row.kind==='mark_quality_check').count,1);
  assert.deepEqual(upstream(o),o.before);
  ...
  if(mode==='disabled')assert.equal(sql("select count(*) from local_commerce.preview_manifests where order_id='"+o.id+"';"),'0');
  report.races.push({mode,pids:racePids,statuses,approvalRace,counts,upstream:o.before});
 }
 report.status='PASS';report.task='7.5';report.classification='LOCAL DEVELOPMENT / TEST PERSISTENCE ACCEPTANCE';console.info(JSON.stringify(report));

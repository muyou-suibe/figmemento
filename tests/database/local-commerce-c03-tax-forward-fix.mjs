...
const migrationSignature = "local_commerce.order_commit(text,text,text,text,uuid,text,timestamptz,text,uuid,integer,text,text,text,timestamptz,timestamptz,jsonb)";
const expectedVolume = "supabase_db_figmemento-local-commerce";
const mode = process.argv[2];

assert.ok(mode === "--rollback-only" || mode === "--apply-authorized-0040");
...
assert.equal(manifest.schemaVersion, 40);
assert.equal(manifest.migrations.length, 40);
assert.equal(manifest.migrations.some((entry) => entry.version === 41), false);
for (const migration of manifest.migrations) {
  assert.equal(
    createHash("sha256").update(readFileSync(path.join(commerceRoot, "migrations", migration.filename))).digest("hex"),
    migration.checksum,
  );
}
...
const forwardWrapper = wrappers.at(-1);
assert.ok(forwardWrapper);

const containerIds = command("docker", ["ps", "--filter", `label=com.supabase.cli.project=${RETAINED_LEDGER_IDENTITY.projectId}`, "--format", "{{.ID}}"])
  .split("\n")
  .filter(Boolean);
...
assert.match(databases[0].Config.Image, /postgres:17(?:\.|$)/);
...
const functionDefinition = () => sql(`select pg_get_functiondef('${migrationSignature}'::regprocedure);`);
const priorChecksums = () => ledger().slice(0, 39).every((entry, index) => entry.version === index + 1 && entry.checksum === manifest.migrations[index].checksum);
...
assert.equal(priorChecksums(), true);
assert.equal(ledger().length, 39);

if (mode === "--rollback-only") {
  const before = functionDefinition();
  assert.match(before, /amount":null/);
  assert.doesNotMatch(before, /amountCents":null/);
  const body = forwardWrapper.replace(/^.*?\nbegin;\n/s, "").replace(/\ncommit;\s*$/s, "");
  sql(`begin;\n${body}\nrollback;`, 60_000);
  const after = functionDefinition();
  assert.equal(after, before);
  assert.equal(ledger().length, 39);
  assert.equal(sql(`select schema_version from local_commerce.project_identities where project_id=${literal(RETAINED_LEDGER_IDENTITY.projectId)};`), "39");
  console.log(JSON.stringify({ status: "PASS", mode: "rollback-only", projectId: RETAINED_LEDGER_IDENTITY.projectId, postgresMajor: 17, baseline: "39/39", functionUnchanged: true, ledgerUnchanged: true, priorMigrationsImmutable: true, remote: false }));
} else {
  assert.match(functionDefinition(), /amount":null/);
  sql(forwardWrapper, 60_000);
  const after = functionDefinition();
  assert.match(after, /amountCents":null/);
  assert.doesNotMatch(after, /amount":null/);
  const applied = ledger();
  assert.equal(applied.length, 40);
  assert.deepEqual(applied.at(-1), { version: 40, checksum: manifest.migrations.at(-1).checksum });
  ...
  assert.equal(sql(`select local_commerce.verify_project_identity(${literal(RETAINED_LEDGER_IDENTITY.projectId)},${literal(markerDigest)});`), "t");
  assert.equal(sql(`select has_function_privilege('public','${migrationSignature}','execute');`), "f");
  assert.equal(sql(`select has_function_privilege('anon','${migrationSignature}','execute');`), "f");
  assert.equal(sql(`select has_function_privilege('authenticated','${migrationSignature}','execute');`), "f");
  assert.equal(sql(`select has_function_privilege('service_role','${migrationSignature}','execute');`), "t");
  console.log(JSON.stringify({ status: "PASS", mode: "apply-authorized-0040", projectId: RETAINED_LEDGER_IDENTITY.projectId, postgresMajor: 17, ledger: "40/40", pending: 0, markerVerified: true, priorMigrationsImmutable: true, security: "PASS", remote: false }));
}

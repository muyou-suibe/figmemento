...
// Applied 0025 validation. Every synthetic SQL fixture/fault is rolled back.
...
assert.equal(ledger.length,25);assert.equal(manifest.schemaVersion,25);
...
try {sql(`begin;set local statement_timeout='8000ms';${customerPreviewSqlChecks(project,prep.markerDigest)}${customerPreviewFaultSql(project,prep.markerDigest)}${fulfillmentReplaySql()}${productionQualitySql(project,prep.markerDigest)}${adminTimeoutSql(project,prep.markerDigest)} rollback;`);}
finally {assert.equal(snapshot(),before,'complete tables unchanged after rollback');}
console.info(JSON.stringify({status:'PASS',run,ledger:25,pending:0,unchangedTables:tables.length,task:'7.6',reserveFaultPoints:4,readyFaultPoints:4,customerFaultPoints:4,revisionPublicationFaultPoints:8,guestMemberSqlAuthorization:true,utf16Boundaries:true,aclRls:true,historicalUpstreamImmutable:true}));

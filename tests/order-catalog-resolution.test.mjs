...
test("22-24: invalid catalog input remains before order, item, coupon, and Stripe mutations", async () => {
  const { repository, mini } = await fixtureProduct();
  const calls = { orders: 0, orderItems: 0, coupons: 0, stripe: 0 };
  const result = await resolveOrderCatalogItems([
    nativeItem(mini, { variantId: "unknown-variant" }),
  ], repository);
  ...
  assert.deepEqual(calls, { orders: 0, orderItems: 0, coupons: 0, stripe: 0 });

  const routeSource = await readFile(new URL("../app/api/orders/route.ts", import.meta.url), "utf8");
  const resolutionIndex = routeSource.indexOf("resolveOrderCatalogItems(");
  const orderInsertIndex = routeSource.indexOf('.from("orders")\n      .insert(');
  const stripeCallIndex = routeSource.indexOf("const session = await createStripeCheckout(");
  assert.ok(resolutionIndex >= 0 && orderInsertIndex > resolutionIndex && stripeCallIndex > orderInsertIndex);
});
...
test("active order route no longer queries legacy products.price_cents or aggregates by slug", async () => {
  const routeSource = await readFile(new URL("../app/api/orders/route.ts", import.meta.url), "utf8");
  assert.doesNotMatch(routeSource, /select\("id, slug, name, price_cents"\)/);
  assert.doesNotMatch(routeSource, /calculateServerProductSubtotal|Map<string, number>/);
  assert.match(routeSource, /unit_price_cents: item\.unitBasePriceCents/);
});

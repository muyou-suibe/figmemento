...
test("Phase B completes Task 6.3 while migration execution remains in Task 3.9", () => {
  const tasks = readFileSync(
    new URL(
      "../openspec/changes/build-configurable-product-catalog/tasks.md",
      import.meta.url,
    ),
    "utf8",
  );

  assert.match(tasks, /- \[x\] 6\.3 Add Product Option\/Option Value and Variant\/SKU management/);
  assert.match(tasks, /- \[ \] 3\.9 Verify all ordered C1 migrations/);
});

...
export function AdminCustomizationFieldEditor({ product }: { product: CatalogProduct }) {
  ...
  const [surchargeRules, setSurchargeRules] = useState<AdminCustomizationSurchargeRule[]>([]);
  const [supportsSurcharges, setSupportsSurcharges] = useState(false);
  const [historicalRevisions, setHistoricalRevisions] = useState<number[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (configured.value.status === "not_configured") {
        ...
        setSupportsSurcharges(Array.isArray(configured.value.surchargeRules));
        setRevision(null);
      } else {
        setFields(editorFieldsFromConfiguration(configured.value.fields));
        setSurchargeRules([...(configured.value.surchargeRules ?? [])]);
        setHistoricalRevisions([...(configured.value.historicalRevisions ?? [])]);
        setRestoreRevision(null);
        setSupportsSurcharges(Array.isArray(configured.value.surchargeRules));
  ...
  async function save() {
    ...
    try {
      const response = await fetch(`/api/admin/catalog/products/${encodeURIComponent(product.id)}/customization`, {
        ...
        body: JSON.stringify({ expectedCurrentRevision: revision, fields: normalizeCustomizationEditorPositions(fields), ...(supportsSurcharges ? { surchargeRules } : {}) }),
      });
      const result: unknown = await response.json().catch(() => null);
      if (isRecord(result) && result.status === "applied" && isRecord(result.value)) {
        ...
        if (canonical && mappings && typeof result.value.configurationRevision === "string") {
          ...
          if (reconciled) {
            ...
            setSurchargeRules(Array.isArray(result.value.surchargeRules) ? result.value.surchargeRules as AdminCustomizationSurchargeRule[] : []);
            setHistoricalRevisions(Array.isArray(result.value.historicalRevisions) ? result.value.historicalRevisions as number[] : []);
  ...
  async function restore() {
    if (!revision || restoreRevision === null) return;
    setStatus("saving");
    setIssues([]);
    try {
      const response = await fetch(`/api/admin/catalog/products/${encodeURIComponent(product.id)}/customization`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ expectedCurrentRevision: revision, restoreFromRevision: restoreRevision }),
      });
      const result: unknown = await response.json().catch(() => null);
      if (response.ok && isRecord(result) && result.status === "applied" && isRecord(result.value)) {
        const canonical = canonicalFields(result.value);
        if (canonical && typeof result.value.configurationRevision === "string") {
          setFields(editorFieldsFromConfiguration(canonical));
          setSurchargeRules(Array.isArray(result.value.surchargeRules) ? result.value.surchargeRules as AdminCustomizationSurchargeRule[] : []);
          setHistoricalRevisions(Array.isArray(result.value.historicalRevisions) ? result.value.historicalRevisions as number[] : []);
          setRevision(result.value.configurationRevision);
          setRestoreRevision(null);
      ...
      if (isRecord(result) && result.status === "stale_revision") { setStatus("stale"); return; }
      setIssues(safeIssues(result));
      setStatus("error");
  ...
  return <section className={styles.customizationSection}>
    ...
    {status !== "loading" && (hasLoaded || status !== "error") ? <>
      ...
      {supportsSurcharges ? <div className={styles.skuGroup}><strong>Fixed customization surcharges (USD cents)</strong><p>Each active customization field can have one field-present surcharge. Existing rules use their current revision.</p>
        {surchargeRules.map((rule, index) => <div className={styles.skuGrid} key={rule.ruleKey}>
          <label><span>Field</span><select value={rule.fieldId} onChange={(event) => setSurchargeRules((current) => current.map((candidate, item) => item === index ? { ...candidate, fieldId: event.target.value } : candidate))}>{fields.filter((field) => field.identity.kind === "existing" && field.isActive).map((field) => <option key={field.identity.kind === "existing" ? field.identity.id : ""} value={field.identity.kind === "existing" ? field.identity.id : ""}>{field.label}</option>)}</select></label>
          <label><span>Amount (cents)</span><input type="number" min="0" max="2147483647" step="1" value={rule.amountCents} onChange={(event) => setSurchargeRules((current) => current.map((candidate, item) => item === index ? { ...candidate, amountCents: Number(event.target.value) } : candidate))} /></label>
          <button type="button" onClick={() => setSurchargeRules((current) => current.filter((_, item) => item !== index))}>Remove surcharge</button>
        </div>)}
        <button type="button" onClick={() => { const target = fields.find((field) => { const id = field.identity.kind === "existing" ? field.identity.id : null; return id && field.isActive && !surchargeRules.some((rule) => rule.fieldId === id); }); if (target?.identity.kind === "existing") { const fieldId = target.identity.id; setSurchargeRules((current) => [...current, { ruleKey: `new:rule-${crypto.randomUUID()}`, expectedRevision: null, fieldId, amountCents: 0, currency: "USD" }]); } }}>Add surcharge</button>
      </div> : null}
      {historicalRevisions.length > 0 ? <div className={styles.skuGroup}>
        <strong>Restore historical configuration as a new revision</strong>
        <p>History is immutable. Restoration revalidates the selected configuration and surcharge facts.</p>
        <select aria-label="Historical customization revision" value={restoreRevision ?? ""} onChange={(event) => setRestoreRevision(event.target.value ? Number(event.target.value) : null)}>
          <option value="">Select revision</option>
          {historicalRevisions.filter((candidate) => String(candidate) !== revision).map((candidate) => <option key={candidate} value={candidate}>Revision {candidate}</option>)}
        </select>
        <button type="button" disabled={restoreRevision === null || status === "saving" || status === "stale"} onClick={() => void restore()}>Restore as new revision</button>
      </div> : null}
...
function FieldEditor({ field, availableFields, index, total, onChange, onMove, onDeactivate }: { field: EditorField; availableFields: readonly EditorField[]; index: number; total: number; onChange: (field: EditorField) => void; onMove: (index: number, direction: -1 | 1) => void; onDeactivate: () => void }) {
  ...
  return <fieldset className={styles.skuGroup}>
    ...
    {image ? <label><span>Image guidance (optional)</span><input maxLength={240} value={image.helpText ?? ""}
      onChange={(event) => { const next: ImageCustomizationFieldConstraints = { ...image, helpText: event.target.value }; if (!next.helpText?.trim()) delete next.helpText;
        update({ constraints: next }); }} /></label> : null}
    ...
    {numeric ? <div className={styles.skuGrid}><label><span>Minimum</span><input inputMode="decimal" value={numeric.min} onChange={(event) => update({ constraints: { ...numeric, min: event.target.value } })} /></label><label><span>Maximum</span><input inputMode="decimal" value={numeric.max} onChange={(event) => update({ constraints: { ...numeric, max: event.target.value } })} /></label><label><span>Step</span><input inputMode="decimal" value={numeric.step} onChange={(event) => update({ constraints: { ...numeric, step: event.target.value } })} /></label><label><span>Help text (optional)</span><input value={numeric.helpText ?? ""} onChange={(event) => update({ constraints: { ...numeric, ...(event.target.value.trim() ? { helpText: event.target.value } : {}) } })} /></label></div> : null}
    {genericFile ? <div className={styles.skuGrid}><label><span>Maximum bytes</span><input type="number" min="1" max="20971520" value={genericFile.maxBytes} onChange={(event) => update({ constraints: { ...genericFile, maxBytes: Number(event.target.value) } })} /></label><label><span>Minimum files</span><input type="number" min="0" max="3" value={genericFile.minFileCount} onChange={(event) => update({ constraints: { ...genericFile, minFileCount: Number(event.target.value) } })} /></label><label><span>Maximum files</span><input type="number" min="1" max="3" value={genericFile.maxFileCount} onChange={(event) => update({ constraints: { ...genericFile, maxFileCount: Number(event.target.value) } })} /></label><label><span>Help text (optional)</span><input value={genericFile.helpText ?? ""} onChange={(event) => update({ constraints: { ...genericFile, ...(event.target.value.trim() ? { helpText: event.target.value } : {}) } })} /></label><label className={styles.mimeField}><span>Allowed types</span>{(["application/pdf", "text/plain"] as const).map((mime) => <span key={mime}><input type="checkbox" checked={genericFile.allowedMimeTypes.includes(mime)} onChange={(event) => update({ constraints: { ...genericFile, allowedMimeTypes: event.target.checked ? [...genericFile.allowedMimeTypes, mime] : genericFile.allowedMimeTypes.filter((value) => value !== mime) } })} />{mime}</span>)}</label></div> : null}
...
function RuleEditor({ field, availableFields, onChange }: { field: EditorField; availableFields: readonly EditorField[]; onChange: (rules?: CustomizationFieldRules) => void }) {
  const candidates = availableFields.filter((candidate): candidate is EditorField & { identity: { kind: "existing"; id: string; code: string } } => candidate.identity.kind === "existing" && candidate.isActive && candidate.identity.id !== (field.identity.kind === "existing" ? field.identity.id : ""));
  const first = candidates[0];
  const defaultPredicate = (): CustomizationPredicate | undefined => first ? { kind: "field_present", fieldId: first.identity.id } : undefined;
  const renderRule = (label: string, current: CustomizationPredicate | undefined, key: "requiredWhen" | "visibleWhen") => {
    ...
    return <div><strong>{label}</strong><button type="button" onClick={() => updateRule(current ? undefined : defaultPredicate())}>{current ? "Remove rule" : "Add rule"}</button>{current ? <PredicateEditor predicate={current} candidates={candidates} depth={1} onChange={updateRule} /> : null}</div>;
  };
  return <div className={styles.skuGrid}><div className={styles.full}><strong>Conditional rules</strong><span>Bounded server-evaluated predicates; publication validates references, cycles and limits.</span></div>{renderRule("Required when", field.rules?.requiredWhen, "requiredWhen")}{renderRule("Visible when", field.rules?.visibleWhen, "visibleWhen")}</div>;
}

function PredicateEditor({ predicate, candidates, depth, onChange }: { predicate: CustomizationPredicate; candidates: readonly (EditorField & { identity: { kind: "existing"; id: string; code: string } })[]; depth: number; onChange: (predicate: CustomizationPredicate) => void }) {
  const choices = candidates.filter((candidate) => candidate.kind === "single_select" || candidate.kind === "multi_select");
  const make = (kind: CustomizationPredicate["kind"]): CustomizationPredicate => {
    if (kind === "all" || kind === "any") return { kind, predicates: [{ kind: "field_present", fieldId: candidates[0]?.identity.id ?? "" }] };
    if (kind === "not") return { kind, predicate: { kind: "field_present", fieldId: candidates[0]?.identity.id ?? "" } };
    if (kind === "field_present") return { kind, fieldId: candidates[0]?.identity.id ?? "" };
    const choiceField = choices.find((candidate) => candidate.kind === (kind === "single_select_is" ? "single_select" : "multi_select"));
    const constraints = choiceField?.constraints as SingleSelectCustomizationFieldConstraints | MultiSelectCustomizationFieldConstraints | undefined;
    return { kind, fieldId: choiceField?.identity.id ?? "", choiceId: constraints?.choices.find((choice) => choice.isActive)?.id ?? "" };
  };
  const selectedField = "fieldId" in predicate ? candidates.find((candidate) => candidate.identity.id === predicate.fieldId) : undefined;
  const selectedChoices = selectedField && (selectedField.kind === "single_select" || selectedField.kind === "multi_select")
    ? (selectedField.constraints as SingleSelectCustomizationFieldConstraints | MultiSelectCustomizationFieldConstraints).choices.filter((choice) => choice.isActive) : [];
  return <div className={styles.full}>
    <label><span>Predicate</span><select value={predicate.kind} onChange={(event) => onChange(make(event.target.value as CustomizationPredicate["kind"]))}>
      <option value="field_present">Field present</option><option value="single_select_is">Single select is</option><option value="multi_select_contains">Multi select contains</option>
      {depth < 4 ? <><option value="all">All</option><option value="any">Any</option><option value="not">Not</option></> : null}
    </select></label>
    {"fieldId" in predicate ? <label><span>Referenced field</span><select value={predicate.fieldId} onChange={(event) => {
      const next = candidates.find((candidate) => candidate.identity.id === event.target.value);
      if (predicate.kind === "field_present") onChange({ ...predicate, fieldId: event.target.value });
      else {
        const constraints = next?.constraints as SingleSelectCustomizationFieldConstraints | MultiSelectCustomizationFieldConstraints | undefined;
        onChange({ ...predicate, fieldId: event.target.value, choiceId: constraints?.choices.find((choice) => choice.isActive)?.id ?? "" });
      }
    }}>{candidates.filter((candidate) => predicate.kind === "field_present" || candidate.kind === (predicate.kind === "single_select_is" ? "single_select" : "multi_select")).map((candidate) => <option key={candidate.identity.id} value={candidate.identity.id}>{candidate.label}</option>)}</select></label> : null}
    {"choiceId" in predicate ? <label><span>Choice</span><select value={predicate.choiceId} onChange={(event) => onChange({ ...predicate, choiceId: event.target.value })}>{selectedChoices.map((choice) => <option key={choice.id} value={choice.id}>{choice.label}</option>)}</select></label> : null}
    {(predicate.kind === "all" || predicate.kind === "any") ? <div>{predicate.predicates.map((child, index) => <div key={index}><PredicateEditor predicate={child} candidates={candidates} depth={depth + 1} onChange={(next) => onChange({ ...predicate, predicates: predicate.predicates.map((item, itemIndex) => itemIndex === index ? next : item) })} /><button type="button" disabled={predicate.predicates.length <= 1} onClick={() => onChange({ ...predicate, predicates: predicate.predicates.filter((_, itemIndex) => itemIndex !== index) })}>Remove child</button></div>)}<button type="button" disabled={predicate.predicates.length >= 32} onClick={() => onChange({ ...predicate, predicates: [...predicate.predicates, make("field_present")] })}>Add child</button></div> : null}
    {predicate.kind === "not" ? <PredicateEditor predicate={predicate.predicate} candidates={candidates} depth={depth + 1} onChange={(next) => onChange({ ...predicate, predicate: next })} /> : null}
  </div>;
}

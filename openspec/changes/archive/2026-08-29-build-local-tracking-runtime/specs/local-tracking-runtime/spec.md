...
Local Tracking SHALL be available only when `LOCAL_TRACKING_SOURCE=local_fake` is explicitly selected in a development/test runtime. An absent source SHALL disable the runtime, and a production runtime SHALL reject it. Tracking Apply MUST remain blocked until Local Order is finally approved, Local Payment is applied and verified, and Local Fulfillment is applied and verified through canonical `quality_check`. A source failure MUST NOT fall back to the local fixture, and no fake paid, Fulfillment, or Quality Check bypass may activate Tracking.

#### Scenario: Tracking planning remains allowed while dependencies are incomplete
- **WHEN** Local Order is 26/30 PARTIAL, Local Payment is planning-complete but unapplied, and Local Fulfillment is planning-complete but unapplied
- **THEN** Tracking planning may exist, but Tracking implementation and workflow execution remain blocked

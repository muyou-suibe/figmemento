...
occurred; it does not create or imply a Payment entity, payment attempt, payment
session, PaymentIntent, authorization, capture, or webhook state. Its local total
is a development/test arithmetic summary with tax status
...
The browser response SHALL contain
only the safe public projection needed for the local success page. Authorization
failure, unknown reference, unavailable local runtime, or post-restart loss
MUST return a bounded response without revealing whether another guest's Order
exists.
...
local runtime. The runtime SHALL not call or write Supabase, migrations, Stripe,
PayPal, webhooks, email, shipping providers, production storage, Admin Orders,
or existing production/legacy Order lookup interfaces.
...
digital `digital-portrait` fixture SHALL NOT be forced through shipping Checkout,
and this change SHALL NOT add Digital Checkout, fake physical text-only Products,
or delivery behavior merely to broaden Local Order test coverage.
